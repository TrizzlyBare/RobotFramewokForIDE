...
try:
    ...
    teacher_output.write("TURTLE_STATE_BEGIN\n")
    ...
    # Log turtle count
    teacher_output.write(f"Number of turtles: {len(turtles)}\n")
    
    # Log details for each turtle
    for i, turt in enumerate(turtles):
        teacher_output.write(f"\nTurtle {i+1}:\n")
        
        # Get position
        try:
            pos = turt.position()
            teacher_output.write(f"Position: {pos[0]}, {pos[1]}\n")
        except Exception as e:
            teacher_output.write(f"Position: Error {e}\n")
        
        # Get heading
        try:
            teacher_output.write(f"Heading: {turt.heading()}\n")
        except Exception as e:
            teacher_output.write(f"Heading: Error {e}\n")
        
        # Get pen state
        try:
            teacher_output.write(f"Pen down: {turt.isdown()}\n")
        except Exception as e:
            teacher_output.write(f"Pen down: Error {e}\n")
        
        # Get pen color
        try:
            teacher_output.write(f"Pen color: {turt.pencolor()}\n")
        except Exception as e:
            teacher_output.write(f"Pen color: Error {e}\n")
        
        # Get fill color
        try:
            teacher_output.write(f"Fill color: {turt.fillcolor()}\n")
        except Exception as e:
            teacher_output.write(f"Fill color: Error {e}\n")
        
        # Get pen size
        try:
            teacher_output.write(f"Pen size: {turt.pensize()}\n")
        except Exception as e:
            teacher_output.write(f"Pen size: Error {e}\n")
        
        # Get visibility
        try:
            teacher_output.write(f"Visible: {turt.isvisible()}\n")
        except Exception as e:
            teacher_output.write(f"Visible: Error {e}\n")
    
    teacher_output.write("TURTLE_STATE_END\n")
    ...
    output_str = teacher_output.getvalue()
    ...
    teacher_output.write(f"ERROR: {error_msg}\n\n")
    teacher_output.write("Traceback:\n")
    
    import traceback
    traceback_io = io.StringIO()
    traceback.print_exc(file=traceback_io)
    teacher_output.write(traceback_io.getvalue())
    
    # Print the error output
    error_output = teacher_output.getvalue()
